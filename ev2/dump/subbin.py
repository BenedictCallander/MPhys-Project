...
class subhalo:
    ...
    def calculate_median_metallicity(self,dfin):
        # Create an empty dataframe to store the results
        data = dfin.copy()
        result = pd.DataFrame(columns=['rad', 'net'])
    
    #  Iterate through the radial bins
        radial_bins = np.arange(0,10,0.1)
        for bin_start in radial_bins:
            # Calculate the end of the bin
            bin_end = bin_start + 0.1
            
            # Extract the rows of the dataframe that fall within the current bin
            bin_data = data[(data['rad'] >= bin_start) & (data['rad'] < bin_end)]
            
            # Calculate the median metallicity for the rows in the current bin
            median_metallicity = bin_data['met'].median()
            
            # Add a row to the result dataframe with the radial bin and median metallicity
            result = result.append({'rad': f'{bin_start:.1f} - {bin_end:.1f}', 'met': median_metallicity}, ignore_index=True)
        print(result)
        return result
...
test = subhalo('TNG50-1',99,275545)
test.galcen()
test.ang_mom_align('gas')
test.rad_transform()
df = test.df_gen('gas','comb')
dfg2=test.combfilter(df,10)
dfg3=test.calculate_median_metallicity(dfg2)
