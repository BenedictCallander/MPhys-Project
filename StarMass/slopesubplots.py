...
fig,axs = plt.subplots(1,3,figsize=(30,8),dpi=500)

for i in range(3):
    axs[i].set_ylabel("$Log_{10}$Star Formation Rate [$M_\odot / yr$]",fontsize=15)
    axs[i].set_xlabel("$Log_{10}$ Galaxy Stellar Mass [$M_\odot$]",fontsize=15)
...
axs[0].set_title("Snapshot 33: z~2.0")
axs[1].set_title("Snapshot 67: z~0.5")
axs[1].set_title("Snapshot 99: z~0.0")

im0=axs[0].scatter(np.log10(df1['mass']),np.log10(df1['sfr']),c=(df1['slope']),cmap='magma',vmin=-0.3,vmax=0.1)
im1=axs[1].scatter(np.log10(df2['mass']),np.log10(df2['sfr']),c=(df2['slope']),cmap='magma',vmin=-0.2,vmax=0.1)#,vmin=1.5,vmax=8)
im2=axs[2].scatter(np.log10(df3['mass']),np.log10(df3['sfr']),c=(df3['slope']),cmap='magma',vmin=-0.2,vmax=0.1)#,vmin=1.5,vmax=8)
...
fig.suptitle("Redshift progression of galaxy $M_*-SFR$ Relation", fontsize=20)
