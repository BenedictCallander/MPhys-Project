...
class galaxy:
    ...
    def split_fit(self,dfin):
        radfilt1 = dfin[dfin['rad']<3]
        radfilt2 = dfin[dfin['rad']>3]
        popt,pcov = curve_fit(linear_fit,radfilt1['rad'],radfilt1['met'])
        popt2,pcov2 = curve_fit(linear_fit,radfilt2['rad'],radfilt2['met'])
        dfin.sort_values(by='rad',inplace = True)
        medfit = medfilt(dfin['met'],kernel_size=3) 
        plt.figure(figsize = (15,10))
        plt.plot(dfin['rad'],medfit, 'g-')
        plt.plot(radfilt1['rad'],linear_fit(radfilt1['rad'],*popt))
        plt.plot(radfilt2['rad'],linear_fit(radfilt2['rad'],*popt2))
